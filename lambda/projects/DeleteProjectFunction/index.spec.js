...
const deleteProjectFunction = require("./index");

// jestのマニュアルモック利用
jest.mock('project-dao');

describe('プロジェクト削除 正常系テスト', () => {
  ...
  test('Lambdaハンドラのテスト 削除データ有', async () => {
    ...
    // 処理呼び出し
    const response = await deleteProjectFunction.lambda_handler(event);
