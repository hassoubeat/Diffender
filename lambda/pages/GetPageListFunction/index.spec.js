// 環境変数の設定
process.env.DIFFENDER_DYNAMODB_TABLE_NAME　= "dummy";

const projectDao = require('project-dao');
const pageDao = require('page-dao');
const getPageListFunction = require("./index");

// jestのマニュアルモック利用
jest.mock('project-dao');
jest.mock('page-dao');

describe('プロジェクト一覧取得 正常系テスト', () => {

  test('Lambdaハンドラのテスト', async () => {

    // マニュアルモックの上書き
    projectDao.getProject = () => {
      return { 
        id:'Project-1' ,
        name: 'プロジェクト1', 
        description: 'example.com1のテスト', 
        projectTieUserId: '8c32116d-5c8c-48c0-8264-1df53434b503' 
      };
    }

    pageDao.getPageList = () => {
      return [
        { 
          id:'Page-1' ,
          name: 'ページ1', 
          description: 'example.com1のテスト', 
          pageTieUserId: '8c32116d-5c8c-48c0-8264-1df53434b503', 
          pageTieProjectId: 'Project-1' 
        },
        { 
          id:'Page-2' ,
          name: 'ページ2', 
          description: 'example.com1のテスト', 
          projectTieUserId: '8c32116d-5c8c-48c0-8264-1df53434b503' ,
          pageTieProjectId: 'Project-1' 
        },
      ]
    }

    // 投入データの生成
    const event = {
      headers: {
        Authorization: "eyJraWQiOiJEQU5keFd6WXhWazZmamhKYThHVlwvM3lOZVwvUlJuXC9MaGZUOWJUenNWTmdFPSIsImFsZyI6IlJTMjU2In0..AIt1zvRhjThea_wMReS0NyO1Zcm228FmiJGyzHyikQj0pkMiM2vRJ01WMkNOdj3_MI-FEQG1wL8qI_DEkiS_yr1g1CECcRmGe9yQjt0EdHrcApju3MJEp1S4TVEC3vkx1_FhUCJYNad34r9Hp-b388d4rZde8E_7643v1R05wTblfjHneVtjF1KnDCEq8azlUfGX5s1OWrsZCq7UJygnIxbNyALav_GGRxgLb9Pu5_bsNmDKwePDWLvDjBY6MQjV4Oa_WfQqFdJZJvYApqp9_7E9nXCLWvywvPP_jEzPLhIGCjyzlWNzbj-FDO3UvwTt_kpopWtgHEhNOGjadl-hsA"
      },
      pathParameters: {
        projectId: "Project-1"
      }
    };

    // 処理呼び出し
    const response = await getPageListFunction.lambda_handler(event);
    expect(response).toEqual(
      {
        statusCode: 200,
        body: "[{\"id\":\"Page-1\",\"name\":\"ページ1\",\"description\":\"example.com1のテスト\",\"pageTieUserId\":\"8c32116d-5c8c-48c0-8264-1df53434b503\",\"pageTieProjectId\":\"Project-1\"},{\"id\":\"Page-2\",\"name\":\"ページ2\",\"description\":\"example.com1のテスト\",\"projectTieUserId\":\"8c32116d-5c8c-48c0-8264-1df53434b503\",\"pageTieProjectId\":\"Project-1\"}]",
        headers: {
          'Access-Control-Allow-Headers': '*',
          'Access-Control-Allow-Origin': '*',
          'Access-Control-Allow-Methods': 'OPTIONS, GET'
        }
      }
    );
  });

});

describe('プロジェクト登録処理 異常系テスト', () => {

  test('Lambdaハンドラのテスト バリデーションエラー ヘッダー無し', async () => {

    // 投入データの生成
    const event = {
    };

    // 処理呼び出し
    const response = await getPageListFunction.lambda_handler(event);
    expect(response).toEqual(
      {
        statusCode: 500,
        headers: {
          'Access-Control-Allow-Headers': '*',
          'Access-Control-Allow-Origin': '*',
          'Access-Control-Allow-Methods': 'OPTIONS, GET'
        },
        body: `{"message":"Cannot read property 'Authorization' of undefined"}`
      }
    );
  });

  test('Lambdaハンドラのテスト 他人のリソースへのアクセス', async () => {

    // マニュアルモックの上書き
    projectDao.getProject = () => {
      return { 
        id:'Project-1' ,
        name: 'プロジェクト1', 
        description: 'example.com1のテスト', 
        projectTieUserId: '111111111-111111-111111111' 
      };
    }

    // 投入データの生成
    const event = {
      headers: {
        Authorization: "eyJraWQiOiJEQU5keFd6WXhWazZmamhKYThHVlwvM3lOZVwvUlJuXC9MaGZUOWJUenNWTmdFPSIsImFsZyI6IlJTMjU2In0..AIt1zvRhjThea_wMReS0NyO1Zcm228FmiJGyzHyikQj0pkMiM2vRJ01WMkNOdj3_MI-FEQG1wL8qI_DEkiS_yr1g1CECcRmGe9yQjt0EdHrcApju3MJEp1S4TVEC3vkx1_FhUCJYNad34r9Hp-b388d4rZde8E_7643v1R05wTblfjHneVtjF1KnDCEq8azlUfGX5s1OWrsZCq7UJygnIxbNyALav_GGRxgLb9Pu5_bsNmDKwePDWLvDjBY6MQjV4Oa_WfQqFdJZJvYApqp9_7E9nXCLWvywvPP_jEzPLhIGCjyzlWNzbj-FDO3UvwTt_kpopWtgHEhNOGjadl-hsA"
      },
      pathParameters: {
        projectId: "Project-1"
      }
    };

    // 処理呼び出し
    const response = await getPageListFunction.lambda_handler(event);
    expect(response).toEqual(
      {
        statusCode: 401,
        headers: {
          'Access-Control-Allow-Headers': '*',
          'Access-Control-Allow-Origin': '*',
          'Access-Control-Allow-Methods': 'OPTIONS, GET'
        },
        body: `{"message":"Unauthorized resource."}`
      }
    );
  });

});